      ...
      <div className={`w-full max-w-md rounded-2xl bg-gradient-to-b ${rStyle.bg} border ${rStyle.border} shadow-2xl ${rStyle.glow} overflow-hidden`}>
        ...
        <div className="p-6 space-y-3">
          <div className="flex items-center gap-3 text-sm">
            <span className="text-gray-500 w-16">Wu Xing</span>
            <span className={`font-semibold ${elColor}`}>
              {pet.bones.dominantElement} · Wood{pet.bones.bazi.elementDistribution.wood} Fire{pet.bones.bazi.elementDistribution.fire} Earth{pet.bones.bazi.elementDistribution.earth} Metal{pet.bones.bazi.elementDistribution.metal} Water{pet.bones.bazi.elementDistribution.water}
            </span>
          </div>
          <div className="flex items-center gap-3 text-sm">
          ...
          <div className="flex items-center gap-3 text-sm">
            <span className="text-gray-500 w-16">Tarot</span>
            <span className="text-white">
              {pet.bones.tarot.name}{pet.bones.tarot.upright ? " Upright" : " Reversed"} — <em className="text-gray-300">{pet.bones.tarot.trait}</em>
            </span>
          </div>
          <div className="flex items-center gap-3 text-sm">
            <span className="text-gray-500 w-16">BaZi</span>
        ...
        <div className="px-6 pb-6 text-xs text-gray-600 flex justify-between">
          <span>{new Date(pet.meta.createdAt).toLocaleDateString("zh-CN")}</span>
